...
class DataBaseInitializer:
    def __init__(self, connection, engine, base):
    ...
    async def _create_database_if_not_exists(self):
        default_engine = create_async_engine(
            url=self.connection.get_default_url(), poolclass=NullPool
        )
        async with default_engine.connect() as connection:
            async with connection.begin():
                ...
                if (self.connection.config.name,) not in existing_databases:
                    await connection.execute(text("COMMIT"))
                    await connection.execute(
                        text(f"CREATE DATABASE {self.connection.config.name}")
                    )
